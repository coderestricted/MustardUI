...
class MustardUI_Settings(bpy.types.PropertyGroup):
    ...
    def addon_version_check(addon_name):
        try:
            ...
            print("MustardUI - " + addon_name + " version is " + str(version[0]) + "." + str(version[1]) + "." + str(
                version[2]) + ".")
            return version
        except:
            return (-1, -1, -1)
        ...
        if self.type in [2, 3, 4, 5]:
            ...
            for mat in obj.data.materials:
                for j in range(len(mat.node_tree.nodes)):
                    if mat.node_tree.nodes[j].name == "MustardUI Float - " + self.name and mat.node_tree.nodes[
                        j].type == "VALUE":
                        mat.node_tree.nodes["MustardUI Float - " + self.name].outputs[
                            0].default_value = self.body_float_value
                    if mat.node_tree.nodes[j].name == "MustardUI BigFloat - " + self.name and mat.node_tree.nodes[
                        j].type == "VALUE":
                        mat.node_tree.nodes["MustardUI BigFloat - " + self.name].outputs[
                            0].default_value = self.body_big_float_value
                    elif mat.node_tree.nodes[j].name == "MustardUI Bool - " + self.name and mat.node_tree.nodes[
                        j].type == "VALUE":
                        mat.node_tree.nodes["MustardUI Bool - " + self.name].outputs[
                            0].default_value = self.body_bool_value
                    elif mat.node_tree.nodes[j].name == "MustardUI - " + self.name and mat.node_tree.nodes[
                        j].type == "RGB":
...
def mustardui_additional_option_bool_update(self, context):
    try:
        ...
        print(
            "MustardUI - Can not find the property. Re-run the Check Additional Option operator in the Configuration menu to solve this")
                ...
                for modifier in rig_settings.model_body.modifiers:
                    if modifier.type == "MASK" and obj.name in modifier.name:
                        modifier.show_viewport = ((
                                                              collection.collection.name == outfits_list or obj.MustardUI_outfit_lock) and not obj.hide_viewport and rig_settings.outfits_global_mask)
        ...
        if len(armature_settings.layers) > 0:
            outfit_armature_layers = [x for x in range(0, 32) if
                                      armature_settings.layers[x].outfit_switcher_enable and armature_settings.layers[
                                          x].outfit_switcher_collection != None]
            ...
            for i in outfit_armature_layers:
                if armature_settings.layers[i].outfit_switcher_object == None:
                    armature_settings.layers[i].show = not armature_settings.layers[
                    ...
                    for object in [x for x in armature_settings.layers[i].outfit_switcher_collection.objects]:
                        if object == armature_settings.layers[i].outfit_switcher_object:
                            armature_settings.layers[i].show = not bpy.data.objects[object.name].hide_viewport and not \
        ...
        for collection in self.outfits_collections:
            for obj in collection.collection.objects:
                ...
                for modifier in self.model_body.modifiers:
                    if modifier.type == "MASK" and obj.name in modifier.name:
                        modifier.show_viewport = ((
                                                              collection.collection.name == self.outfits_list or obj.MustardUI_outfit_lock) and not obj.hide_viewport and self.outfits_global_mask)
                        modifier.show_render = ((
                                                            collection.collection.name == self.outfits_list or obj.MustardUI_outfit_lock) and not obj.hide_viewport and self.outfits_global_mask)
...
class MustardUI_ArmatureLayer(bpy.types.PropertyGroup):
    ...
    def outfit_switcher_poll_collection(self, object):
        ...
        return object in [x.collection for x in
                          rig_settings.outfits_collections] or object == rig_settings.extras_collection
        ...
        for mat in rig_settings.model_body.data.materials:
            for j in range(len(mat.node_tree.nodes)):
                if "MustardUI Float" in mat.node_tree.nodes[j].name and mat.node_tree.nodes[j].type == "VALUE":
                    mustardui_add_option_item_body(rig_settings.body_additional_properties,
                                                   [mat.node_tree.nodes[j].name[len("MustardUI Float - "):],
                                                    mat.node_tree.nodes[j].outputs[0].default_value, 3])
                elif "MustardUI BigFloat" in mat.node_tree.nodes[j].name and mat.node_tree.nodes[j].type == "VALUE":
                    mustardui_add_option_item_body(rig_settings.body_additional_properties,
                                                   [mat.node_tree.nodes[j].name[len("MustardUI BigFloat - "):],
                                                    mat.node_tree.nodes[j].outputs[0].default_value, 4])
        ...
        if rig_settings.model_body.data.shape_keys != None:
            for shape_key in rig_settings.model_body.data.shape_keys.key_blocks:
                if "MustardUI Float" in shape_key.name:
                    mustardui_add_option_item_body(rig_settings.body_additional_properties,
                                                   [shape_key.name[len("MustardUI Float - "):], shape_key.value, 1])
                elif "MustardUI Bool" in shape_key.name:
                    mustardui_add_option_item_body(rig_settings.body_additional_properties,
                                                   [shape_key.name[len("MustardUI Bool - "):], shape_key.value, 0])
        ...
        for collection in collections:
            for obj in collection.objects:
                if obj.type == "MESH":
                    for mat in obj.data.materials:
                        for j in range(len(mat.node_tree.nodes)):
                            if "MustardUI Float" in mat.node_tree.nodes[j].name and mat.node_tree.nodes[
                                j].type == "VALUE":
                                mustardui_add_option_item(obj.mustardui_additional_options,
                                                          [mat.node_tree.nodes[j].name[len("MustardUI Float - "):],
                                                           'bpy.data.materials[\'' + mat.name + '\'].node_tree.nodes[\'' +
                                                           mat.node_tree.nodes[j].name + '\'].outputs[0]',
                                                           'default_value', obj, 3])
                            elif "MustardUI BigFloat" in mat.node_tree.nodes[j].name and mat.node_tree.nodes[
                                j].type == "VALUE":
                                mustardui_add_option_item(obj.mustardui_additional_options,
                                                          [mat.node_tree.nodes[j].name[len("MustardUI BigFloat - "):],
                                                           'bpy.data.materials[\'' + mat.name + '\'].node_tree.nodes[\'' +
                                                           mat.node_tree.nodes[j].name + '\'].outputs[0]',
                                                           'default_value', obj, 4])
                            elif "MustardUI Bool" in mat.node_tree.nodes[j].name and mat.node_tree.nodes[
                                j].type == "VALUE":
                                mustardui_add_option_item(obj.mustardui_additional_options,
                                                          [mat.node_tree.nodes[j].name[len("MustardUI Bool - "):],
                                                           'bpy.data.materials[\'' + mat.name + '\'].node_tree.nodes[\'' +
                            ...
                            if "MustardUI Float" in shape_key.name:
                                mustardui_add_option_item(obj.mustardui_additional_options,
                                                          [shape_key.name[len("MustardUI Float - "):],
                                                           'bpy.data.objects[\'' + obj.name + '\'].data.shape_keys.key_blocks[\'' + shape_key.name + '\']',
                                                           'value', obj, 1])
                            elif "MustardUI Bool" in shape_key.name:
                                mustardui_add_option_item(obj.mustardui_additional_options,
    ...
    def execute(self, context):
        ...
        if not obj.MustardUI_enable:
            ...
            if rig_settings.model_body.scale[0] != 1. or rig_settings.model_body.scale[1] != 1. or \
                ...
                if settings.debug:
                    print(
                        'MustardUI - Configuration Warning - The selected body mesh seems not to have the scale applied.\n This might generate issues with the tools.')
            ...
            for i in [x for x in range(0, 32) if armature_settings.config_layer[x]]:
                ...
                if armature_settings.layers[i].outfit_switcher_enable and armature_settings.layers[
                    i].outfit_switcher_collection == None:
                    warnings = warnings + 1
                    if settings.debug:
            ...
            for i in [x for x in range(0, 32) if armature_settings.config_layer[x]]:
                ...
                if armature_settings.layers[i].name.find(mirror_string) > 1:
                    ...
                    if len(mirror_layer) > 0:
                        ...
                        if settings.debug:
                            print(
                                'MustardUI - Configuration Warning - Can not find a mirror layer. Mirror has been disabled.')
                        armature_settings.layers[i].mirror = False
                else:
                    warnings = warnings + 1
                    if settings.debug:
            ...
            if rig_recognized < 2:
                ...
                if settings.debug:
                    print(
                        'MustardUI - Configuration Warning - The rig has multiple rig types. This might create problems in the UI')

            # Check MHX requirements for IK/FK support
            if armature_settings.enable_ik_fk and (
            ...
            settings.status_diffeomorphic_version[2]) >= (1, 6, 0):
                ...
                if settings.debug:
                    print(
                        'MustardUI - Configuration Warning - IK/FK support requested for MHX rig, but from Diffeomorphic 1.6.0 it has been moved to MHX independent panel')

            if armature_settings.enable_ik_fk and rig_settings.model_rig_type == "mhx" and settings.status_diffeomorphic < 2:
                warnings = warnings + 1
                if settings.debug:
                    print(
                        'MustardUI - Configuration Warning - IK/FK support requested for MHX rig, but Diffeomorphic is not installed')
            ...
            if armature_settings.enable_ik_fk and rig_settings.model_rig_type != "mhx":
                ...
                if settings.debug:
                    print(
                        'MustardUI - Configuration Warning - IK/FK support requested for non-MHX rig. The IK/FK option will be switched off')
            ...
            if tools_settings.lips_shrinkwrap_enable and not rig_settings.model_rig_type in ['arp', 'mhx']:
                ...
                if settings.debug:
                    print(
                        'MustardUI - Configuration Warning - Lips shrinkwrap requested for a rig which is not ARP or MHX. The tool has been disabled')

            if warnings > 0:
                if settings.debug:
                    print("\n\n")
                self.report({'WARNING'},
                            'MustardUI - Some warning were generated during the configuration. Enable Debug mode and check the console for more informations')
            else:
                if settings.debug:
...
class MustardUI_OutfitVisibility(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if rig_settings.extras_collection != None:
            rig_settings.extras_collection.hide_viewport = len(
                [x for x in rig_settings.extras_collection.objects if not x.hide_render]) == 0
...
class MustardUI_Tools_Physics_DeleteItem(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if self.cage_object_name == "":
            ...
            self.report({'WARNING'},
                        "MustardUI: Physics Item list cleaned from un-referenced cages. The modifiers could not be cleaned.")
            ...
            if rig_settings.body_additional_properties_number > 0:
                ...
                if len(rig_settings.body_additional_properties_sections) == 0:
                    ...
                    for i_sec in sorted([x for x in range(0, section_len)],
                                        key=lambda x: rig_settings.body_additional_properties_sections[x].id):
            ...
            if len(rig_settings.outfits_collections) > 0:
                ...
                for collection in rig_settings.outfits_collections:
                    if hasattr(collection.collection, 'name'):
                        ...
                        del_col = row.operator("mustardui.delete_outfit", text="",
                                               icon="X").col = collection.collection.name
                ...
                if rig_settings.outfit_additional_options:
                    ...
                    box.label(text="Additional outfit options", icon="PRESET_NEW")
                    box.label(
                        text="  Current properties number: " + str(rig_settings.outfits_additional_properties_number))
        ...
        if not physics_settings.config_collapse:
            ...
            for item in physics_settings.physics_items:
                ...
                try:
                    ...
                    row.label(text=item.cage_object.name[len(
                        rig_settings.model_name + ' Physics - '):] if rig_settings.model_MustardUI_naming_convention else item.cage_object.name)
...
class PANEL_PT_MustardUI_Body(MainPanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if len(rig_settings.body_additional_properties) > 0:
            ...
            additional_properties_sk = sorted(
                [x for x in rig_settings.body_additional_properties if x.type in [0, 1] and x.section == ""],
                key=lambda x: x.name)
            additional_properties_mat = sorted(
                [x for x in rig_settings.body_additional_properties if x.type in [2, 3, 4, 5] and x.section == ""],
                key=lambda x: x.name)
            ...
            for i_sec in sorted([x for x in range(0, len(rig_settings.body_additional_properties_sections))],
                                key=lambda x: rig_settings.body_additional_properties_sections[x].id):
                section = rig_settings.body_additional_properties_sections[i_sec]
                additional_properties_section = sorted(
                    [x for x in rig_settings.body_additional_properties if x.section == section.name],
                    key=lambda x: x.name)
                if len(additional_properties_section) > 0 and (
                        not section.advanced or (section.advanced and settings.advanced)):
                    box = layout.box()
                    box.label(text=section.name,
                              icon=section.icon if (section.icon != "" and section.icon != "NONE") else "DOT")
    ...
    def draw(self, context):
        ...
        if rig_settings.diffeomorphic_emotions_units:
            ...
            if not rig_settings.diffeomorphic_emotions_units_collapse:
                ...
                for morph in [x for x in rig_settings.diffeomorphic_morphs_list if
                              x.type == 0 and rig_settings.diffeomorphic_search.lower() in x.name.lower()]:
        ...
        if rig_settings.diffeomorphic_emotions:
            ...
            if not rig_settings.diffeomorphic_emotions_collapse:
                for morph in [x for x in rig_settings.diffeomorphic_morphs_list if
                              x.type == 1 and rig_settings.diffeomorphic_search.lower() in x.name.lower()]:
                    if hasattr(rig_settings.model_armature_object, '[\"' + morph.path + '\"]'):
                        box.prop(rig_settings.model_armature_object, '[\"' + morph.path + '\"]', text=morph.name)
                    else:
                        row = box.row(align=False)
        ...
        if rig_settings.diffeomorphic_facs_emotions_units:
            ...
            if not rig_settings.diffeomorphic_facs_emotions_units_collapse:
                ...
                for morph in [x for x in rig_settings.diffeomorphic_morphs_list if
                              x.type == 2 and rig_settings.diffeomorphic_search.lower() in x.name.lower()]:
                    if hasattr(rig_settings.model_armature_object, '[\"' + morph.path + '\"]'):
                        box.prop(rig_settings.model_armature_object, '[\"' + morph.path + '\"]', text=morph.name)
                    else:
                        row = box.row(align=False)
        ...
        if rig_settings.diffeomorphic_facs_emotions:
            ...
            if not rig_settings.diffeomorphic_facs_emotions_collapse:
                ...
                for morph in [x for x in rig_settings.diffeomorphic_morphs_list if
                              x.type == 3 and rig_settings.diffeomorphic_search.lower() in x.name.lower()]:
                    if hasattr(rig_settings.model_armature_object, '[\"' + morph.path + '\"]'):
                        box.prop(rig_settings.model_armature_object, '[\"' + morph.path + '\"]', text=morph.name)
                    else:
                        row = box.row(align=False)
        ...
        if rig_settings.diffeomorphic_body_morphs:
            ...
            if not rig_settings.diffeomorphic_body_morphs_collapse:
                ...
                for morph in [x for x in rig_settings.diffeomorphic_morphs_list if
                              x.type == 4 and rig_settings.diffeomorphic_search.lower() in x.name.lower()]:
                    if hasattr(rig_settings.model_armature_object, '[\"' + morph.path + '\"]'):
                        box.prop(rig_settings.model_armature_object, '[\"' + morph.path + '\"]', text=morph.name)
                    else:
                        row = box.row(align=False)
...
class PANEL_PT_MustardUI_Outfits(MainPanel, bpy.types.Panel):
    ...
    def poll(cls, context):
        ...
        if arm != None:
            ...
            if rig_settings.model_body != None:
                particle_avail = len(
                    rig_settings.model_body.particle_systems) > 0 and rig_settings.particle_systems_enable
            else:
                ...
                if len(bpy.data.collections[rig_settings.outfits_list].objects) > 0:
                    for obj in bpy.data.collections[rig_settings.outfits_list].objects:
                        ...
                        if len(obj.mustardui_additional_options) > 0 and rig_settings.outfit_additional_options:
                            row.prop(bpy.data.objects[obj.name], "mustardui_additional_options_show", toggle=True,
                                     icon="PREFERENCES")
                                ...
                                for aprop in sorted(obj.mustardui_additional_options, key=lambda x: x.type):
                                    ...
                                    if aprop.type in [0, 2]:
                                        ...
                                        try:
                                            ...
                                            row2.prop(settings, 'additional_properties_error', text="", icon="ERROR",
        ...
        if len(rig_settings.model_body.particle_systems) > 0 and rig_settings.particle_systems_enable:
            ...
            for psys in rig_settings.model_body.particle_systems:
                ...
                if psys.settings.mustardui_particle_hair_enable:
                    row2.prop(psys.settings, "mustardui_particle_hair_enable", text="", toggle=True,
                              icon="RESTRICT_RENDER_OFF")
                else:
                    row2.prop(psys.settings, "mustardui_particle_hair_enable", text="", toggle=True,
                              icon="RESTRICT_RENDER_ON")
                if psys.settings.mustardui_particle_hair_enable_viewport:
                    row2.prop(psys.settings, "mustardui_particle_hair_enable_viewport", text="", toggle=True,
    ...
    def draw(self, context):
        ...
        if len(enabled_layers) > 0:
            ...
            for i in sorted([x for x in range(0, 32) if
                ...
                if (armature_settings.layers[i].advanced and settings.advanced) or not armature_settings.layers[
                    i].advanced:
                    if armature_settings.layers[i].mirror and armature_settings.layers[i].mirror_left:
                        ...
                        box.prop(armature_settings.layers[i], "show", text=armature_settings.layers[i].name,
                                 toggle=True)
        ...
        if armature_settings.enable_ik_fk and rig_settings.model_rig_type == "mhx" and rig_settings.model_armature_object != None:
            ...
            if settings.status_diffeomorphic > 1:
                ...
                if (settings.status_diffeomorphic_version[0], settings.status_diffeomorphic_version[1],
                    settings.status_diffeomorphic_version[2]) >= (1, 6, 0) or not hasattr(
                        rig_settings.model_armature_object, '["MhaArmIk_L"]'):
                    return

                box = layout.box()

                row = box.row(align=False)
                row.prop(armature_settings, "ik_fk_collapse",
                         icon="TRIA_DOWN" if not armature_settings.ik_fk_collapse else "TRIA_RIGHT", icon_only=True,
                         emboss=False)
...
class PANEL_PT_MustardUI_SettingsPanel(MainPanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        box.label(text="MustardUI:    " + str(bl_info["version"][0]) + '.' + str(bl_info["version"][1]) + '.' + str(
            bl_info["version"][2]))
        ...
        if (rig_settings.model_rig_type == "arp" and settings.status_rig_tools == 0) or (
                ...
                settings.status_diffeomorphic == 1 and rig_settings.diffeomorphic_support):
            box = layout.box()
...
class PANEL_PT_MustardUI_Links(MainPanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        box.operator('mustardui.openlink', text="MustardUI - Tutorial",
                     icon="WORLD").url = rig_settings.url_MustardUItutorial
        box.operator('mustardui.openlink', text="MustardUI - Report Bug",
                     icon="WORLD").url = rig_settings.url_MustardUI_reportbug
